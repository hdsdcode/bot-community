...
(
    ESCOLHA, NOME, IDADE, ESTADO_CIVIL, TELEFONE, EMAIL, OBJETIVO, LINKEDIN,
...
def validar_linkedin(texto):
    return re.match(r"^(https?:\/\/)?(www\.)?linkedin\.com\/in\/[a-zA-Z0-9_-]+\/?$", texto, re.IGNORECASE) or texto.upper() == 'N'
...
class PDF(FPDF):
    def header(self):
        ...
        # A célula abaixo já ocupa a largura total (0) e centraliza o texto ('C')
...
def gerar_pdf(data):
    ...
    def secao(titulo):
        pdf.ln(4) # Pode ajustar este valor se quiser mais espaço entre as seções
    ...
    if linkedin_url and linkedin_url.upper() != 'N':
        pdf.cell(0, 6, f"LinkedIn: {linkedin_url}", 0, 1)
        pdf.set_x(left_margin)

    secao("Objetivo")
    objetivo_texto = data.get('objetivo', 'Não informado')
    pdf.set_x(left_margin + 5)
    pdf.multi_cell(0, 6, objetivo_texto, 0, 'L')
    pdf.set_x(left_margin)
...
async def ask_text_standard(update: Update, context: ContextTypes.DEFAULT_TYPE, prompt: str, field_key: str, next_state, validation_func=None, error_message: str = "⚠️ Entrada inválida! Tente novamente."):
    ...
    if field_key not in ['faculdade', 'curso', 'situacao', 'ano', 'empresa', 'cargo', 'adm', 'dem', 'atividades', 'resultados', 'instituicao', 'nome_idioma', 'nivel', 'ini', 'fim', 'mei_trabalhos']:
        context.user_data[field_key] = text.title() if field_key not in ['email', 'linkedin'] else text.lower()
    else:
        ...
        if field_key in ['faculdade', 'curso', 'situacao', 'ano']:
            ...
            if current_level == 'graduacao':
                context.user_data.setdefault('graduacoes', [])
                if idx >= len(context.user_data['graduacoes']):
                    context.user_data['graduacoes'].append({})
            elif current_level == 'pos_graduacao':
                context.user_data.setdefault('pos_graduacoes', [])
                if idx >= len(context.user_data['pos_graduacoes']):
                    context.user_data['pos_graduacoes'].append({})
            ...
            context.user_data.setdefault('experiencias', [])
            if idx >= len(context.user_data['experiencias']):
                context.user_data['experiencias'].append({})
            
            if field_key in ['atividades', 'resultados'] and text.upper() == 'N':
                context.user_data['experiencias'][idx][field_key] = ''
            elif field_key in ['atividades', 'resultados']:
            ...
            context.user_data.setdefault('idiomas', [])
            if idx >= len(context.user_data['idiomas']):
                context.user_data['idiomas'].append({})
...
async def start(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    await update.message.reply_text(
        ...
        "*Você pode cancelar a qualquer momento digitando /cancel.*",
        parse_mode='Markdown'
    )
...
async def cursos(update, context):
    ...
    if cursos_texto.upper() == 'N':
        context.user_data['cursos'] = ''
    else:
...
def main():
    ...
    conv_handler = ConversationHandler(
        ...
        states={
            ...
            LINKEDIN: [MessageHandler(filters.TEXT & ~filters.COMMAND, linkedin_profile)],
